#include "byte_stream.hh"

// Dummy implementation of a flow-controlled in-memory byte stream.

// For Lab 0, please replace with a real implementation that passes the
// automated checks run by `make check_lab0`.

// You will need to add private members to the class declaration in `byte_stream.hh`

template <typename... Targs>
void DUMMY_CODE(Targs &&... /* unused */) {}

using namespace std;

ByteStream::ByteStream(const size_t capacity) : buffer(), capacity_(capacity),
                    total_in(0), total_out(0), end_write(false), end_read(false) {
}

size_t ByteStream::write(const string &data) {
    size_t number = (data.length() > (capacity_ - buffer.size()) ? (capacity_ - buffer.size()) : data.length());
    for(size_t index = 0; index < number; index++){
        buffer.push_back(data.at(index));
    }
    total_in += number;
    return number;
}

//! \param[in] len bytes will be copied from the output side of the buffer
string ByteStream::peek_output(const size_t len) const {
    size_t number = (len > buffer.size() ? buffer.size() : len);
    string out = "";
    for(size_t index = 0; index < number; index++){
        out += buffer[index];
    }
    return out;
}

//! \param[in] len bytes will be removed from the output side of the buffer
void ByteStream::pop_output(const size_t len) {
    if(buffer.size() < len){
        set_error();
        return;
    }
    for(size_t index = 0; index < len; index++){
        buffer.pop_front();
    }
    total_out += len;
    return;
}

//! Read (i.e., copy and then pop) the next "len" bytes of the stream
//! \param[in] len bytes will be popped and returned
//! \returns a string
std::string ByteStream::read(const size_t len) {
    string out = "";
    if(buffer.size() < len){
        set_error();
        return "";
    }
    out = peek_output(len);
    pop_output(len);
    return out;
}

void ByteStream::end_input() { end_write = 1; }

bool ByteStream::input_ended() const { return end_write; }

size_t ByteStream::buffer_size() const { return buffer.size(); }

bool ByteStream::buffer_empty() const { return buffer.size() == 0; }

bool ByteStream::eof() const { return (end_write && buffer.size() == 0); }

size_t ByteStream::bytes_written() const { return total_in; }

size_t ByteStream::bytes_read() const { return total_out; }

size_t ByteStream::remaining_capacity() const { return capacity_ - buffer.size(); }
